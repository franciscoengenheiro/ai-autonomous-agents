...
\begin{boldenumerate}
    \item Definição de uma biblioteca denominada ECR (Estímulos, Comportamentos e Respostas) que expõe os mecanismos base de comportamento (ver secção~\ref{subsec:comportamento}), reação (ver secção~\ref{subsec:reacao}), estímulo, resposta, e comportamento composto (ver secção~\ref{subsec:comportamento-composto});
